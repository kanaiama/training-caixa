(function (factory) {
	...
	var getCountdown = function(percircle, options, progressBarColor) {
		...
		var d = new Date(1990, 01, 01, 00, secs/60, 00, 00); // new Date(year, month, day, hours, minutes, seconds, milliseconds)
		...
		function timer() {
		  ...
		  if (secs > 150) percircle.addClass('gt50');
		  if (secs < 150) percircle.removeClass('gt50');
		  ...
		  if (secs <= 0) {
			...
			percircle.html('<span style="font-size: 27px; margin-top: 5px; font-weight:bold;">'+timeUpText+'</span>');
			return;
		  }
		}
		...
		function timerUpdate() {
			...
			percircle.html('<span style="font-size: 20px; margin-top: -45px;"> N\u00FAmero PIN </span> '
					+ '<span style="font-size: 60px; margin-top: 5px;">' + pin + '</span>' 
					+ '<span style="font-size: 20px; margin-top: 45px; ">' + contadorText + '</span>');
			
			// add divs for structure
			$('<div class="slice"><div class="bar" '+progressBarColor+'></div><div class="fill" '+progressBarColor+'></div></div>').appendTo(percircle);

			var rotationDegrees = 1.2 * secs;  // temporary clockwise rotation value fixado em 1,2 que e o valor obtido para 360graus/5minutos
			$('.bar', percircle).css({
